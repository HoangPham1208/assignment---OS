...
/* 
 * pte_set_fpn - Set PTE entry for frame page number
 * @pte   : target page table entry (PTE)
...
int vmap_page_range(struct pcb_t *caller, // process call
                                int addr, // start address which is aligned to pages
...
int vm_map_ram(struct pcb_t *caller, int astart, int aend, int mapstart, int incpgnum, struct vm_rg_struct *ret_rg)
{
   ...
   */
  if(incpgnum > caller->mram->maxsz) ret_alloc = -3000;
  else ret_alloc = alloc_pages_range(caller, incpgnum, &frm_lst);
  
  if (ret_alloc < 0 && ret_alloc != -3000) // < 0 means error code, -3000 means OOM
    return -1;
  ...
  // Map the allocated frames to the given address space
